...
const uniquePeople = (items: Person[]) => Array.from(new Map(items.map(p => [p.id, p])).values())
...
export default function Home() {
  ...
  const parents = useMemo(() => current ? uniquePeople(people.filter(p => p.relationships?.some(r => r.type === "PARENT" && r.toId === current.id) || p.relatedTo?.some(r => r.type === "PARENT" && r.fromId === current.id))) : [], [people, current])
  const children = useMemo(() => current ? uniquePeople(people.filter(p => p.relationships?.some(r => r.type === "PARENT" && r.fromId === current.id) || p.relatedTo?.some(r => r.type === "PARENT" && r.toId === current.id))) : [], [people, current])
  ...
  async function relationship(e: FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) return setError((await res.json()).error || "Could not connect people")
    setModal(null)
    await loadPeople()
  }
